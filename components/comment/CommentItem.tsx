...
export function CommentItem({
  ...
  const [likeCount, setLikeCount] = useState<number>(comment.likedCount)
  ...
  return (
    <article
      ...
      aria-label={`${comment.user?.nickname ?? '匿名'}的评论`}
    >
      <Avatar className="w-10 h-10 flex-shrink-0">
        <AvatarImage src={comment.user?.avatarUrl} alt={comment.user?.nickname || '用户头像'} />

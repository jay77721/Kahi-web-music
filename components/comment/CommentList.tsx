...
function normalizeComments(raw: unknown): CommentResponse {
  const source =
    raw && typeof raw === 'object' && 'data' in raw
      ...
      : raw
  return normalizeCommentResponse(source)
}
...
export function CommentList({ id, type }: CommentListProps) {
  ...
  const fetcher = useCallback(async (): Promise<CommentResponse> => {
    ...
    if (type === 'song') return normalizeComments(await ncmApi.request('/comment/music', params))
    if (type === 'playlist') return normalizeComments(await ncmApi.request('/comment/playlist', params))
    if (type === 'album') return normalizeComments(await ncmApi.request('/comment/album', params))
    if (type === 'mv') return normalizeComments(await ncmApi.request('/comment/mv', params))
